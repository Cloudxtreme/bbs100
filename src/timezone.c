...
/* stupid check to see if we've got all the fields */
		...
		if (cstrncmp(arr[15], "gmtoff=", 7) || cstrncmp(arr[14], "isdst=", 6)) {
			Free(arr);
			fclose(f);
			listdestroy_ZoneInfo(zoneinfo);
			return NULL;
...
/* mind leap seconds ... they usually don't happen at DST transitions, but hey ... */
		if (mk_tm.tm_hour < 0 || mk_tm.tm_hours > 23
		...
		t = mktime(&mk_tm);
		if (t < now) {			/* it's in the past ... never mind */
			Free(arr);
			continue;
		}
		gmtoff = atoi(arr[15]+7);
