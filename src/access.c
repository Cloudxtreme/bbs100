...
char *err_msg = NULL;
	...
	if (in_StringQueue(usr->enemies, u->name) != NULL && !(usr->runtime_flags & RTF_SYSOP))
		err_msg = " <white>--> <red>Is on your enemy list";
	else
	if (u->runtime_flags & RTF_LOCKED)
		err_msg = " <white>--> <red>Is away from the terminal";
	else
	if (in_StringQueue(u->enemies, usr->name) != NULL && !(usr->runtime_flags & RTF_SYSOP))
		err_msg = " <white>--> <red>Has blocked you";
	else
	if ((u->flags & USR_X_DISABLED) && in_StringQueue(u->friends, usr->name) == NULL && !(usr->runtime_flags & RTF_SYSOP))
		err_msg = " <white>--> <red>Has message reception disabled";
	else
	if ((usr->flags & USR_X_DISABLED) && in_StringQueue(usr->friends, u->name) == NULL && !(usr->runtime_flags & RTF_SYSOP))
...
char err_msg[MAX_LINE] = "";
	...
	if (in_StringQueue(usr->enemies, usr->edit_buf) != NULL && !(usr->runtime_flags & RTF_SYSOP)) {
...
int allocated = 0;
	...
	if (in_StringQueue(usr->enemies, name) != NULL && !(usr->runtime_flags & RTF_SYSOP)) {
...
User *u;
	...
	for(sl = (StringList *)usr->recipients->tail; sl != NULL; sl = sl_next) {
		...
		if (!(usr->runtime_flags & RTF_SYSOP)) {
			...
			if ((u->flags & USR_X_DISABLED) && (in_StringQueue(u->friends, usr->name) == NULL)) {
				Print(usr, "<red>Sorry, but <yellow>%s<red> does not wish to receive any messages right now\n", sl->str);
				goto Remove_Checked_Recipient;
			}
			if ((usr->flags & USR_X_DISABLED) && (in_StringQueue(usr->friends, u->name) == NULL)) {
